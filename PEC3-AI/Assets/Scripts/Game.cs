...
public class Game: MonoBehaviour {
	...
	void Start () {
		...
		globalReward = 0.0f;
		avgReward    = 0.0f;
        breakableLeft= breakables.Length;
        StartEpisode();
	}
		...
		for(int i=0; i < ROWS; i++)
			for (int j = 0; j < COLS; j++)
				if (board_int[i, j] == 2) {
                    ...
                    Vector3 pos = new Vector3(j, -0.49f, i);
                    board_go[i, j] = Instantiate (arrowsPrefab, pos, Quaternion.identity);
                }
	...
	void StartEpisode() {
        ...
        if(episode == 0) dataChart += ";Rewards;Avg.Rwd.;Turns" + System.Environment.NewLine;
        else dataChart += episode + ";" + episodeReward + ";" + avgReward + ";" + turn + System.Environment.NewLine;
	...
	void NewTurn() {
        if (episode >= limitEpisodes && !fileCreated)
            if(createExcel) StartCoroutine(CrearArchivoCSV());
        else if (fileCreated) return;

        // 1) Ask the agent to choose an action given the current game state
        int state  = ComputeState();
		int action = agent.ChooseAction(state);

        if (agent.qTable[state, action] > 0)
        {
            SetArrow(state);
        }
        ...
        if (episodeEnds && episode < limitEpisodes)
            StartEpisode();
    ...
    void SetArrow(int state)
    {
        if (board_go[(int)agent.transform.position.z, (int)agent.transform.position.x].transform.childCount > 1)
        {
            float bestPunt = 0;
            int bestAction = 0;
            for (int n = 0; n < 4; n++)
            {
                if (agent.qTable[state, n] > bestPunt)
                {
                    bestPunt = agent.qTable[state, n];
                    bestAction = n;
                }
                board_go[(int)agent.transform.position.z, (int)agent.transform.position.x].transform.GetChild(n).gameObject.SetActive(false);
            }
            board_go[(int)agent.transform.position.z, (int)agent.transform.position.x].transform.GetChild(bestAction).gameObject.SetActive(true);
        }
    }
    ...
    // 3) Multiply both numbers (to get all possible combinations)
    // The state of the game is defined by the elements that may change. There is no need
    // to include the solid walls or the empty cells
    ...
    IEnumerator CrearArchivoCSV()
    {
        fileCreated = true;
        var fileName = "Data/estadisticasMaze.csv ";
        string ruta = Path.Combine(Application.persistentDataPath, fileName);

        if (File.Exists(ruta))
        {
            int n = 0;
            string plainName = ruta.Remove(ruta.Length - 5);
            string newName;
            do
            {
                n++;
                newName = string.Format("{0}({1}){2}", plainName, n, ".csv");
                ruta = Path.Combine(Application.persistentDataPath, newName);
            }
